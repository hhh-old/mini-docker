...
// AddProcess 将指定 PID 添加到 v1 的所有 cgroup 子系统中
func (m *managerV1) AddProcess(pid int) error {
	pidStr := strconv.Itoa(pid)
	for _, subsys := range []string{"memory", "cpu", "pids", "freezer"} {
		if path, ok := m.paths[subsys]; ok {
			if err := WriteFile(path, "cgroup.procs", pidStr); err != nil {
				return fmt.Errorf("添加进程到 %s cgroup 失败: %w", subsys, err)
			}
		}
	}
	return nil
